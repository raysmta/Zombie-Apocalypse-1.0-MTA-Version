...
function giveWeaponCommand(cmd, weapon, amount)
	...
	if not weapon or weapon < 0 or weapon > 46 then
		return
	end
	if not amount then
		amount = 500
...
function setPlayerPosition(x, y, z)
	...
	if not hit then
		...
		setTimer(putCameraAt, 1000, 1, x, y, z)
...
function setColorCommand(cmd, ...)
	...
	for i,color in ipairs({ ... }) do
		colors[i] = color and tonumber(color) or colors[i]
	end
	server.setVehicleColor(vehicle, unpack(colors))
end
...
function mainWndShow()
	updateTimer = setTimer(updateGUI, 2000, 0)
	updateGUI(true)
end
...
addEventHandler('onClientResourceStop', g_ResRoot, function() showCursor(false) end)

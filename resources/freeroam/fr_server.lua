...
function giveMeWeapon(weapon, amount)
	if table.find(getOption('weapons.disallowed'), weapon) and not isPlayerInACLGroup(source, 'Admin') then
		errMsg(getWeaponNameFromID(weapon) .. 's are not allowed', source)
	else
...
addEventHandler('onServerCall', g_Root,
	function(fnName, ...)
		...
		if fnInfo and ( fnInfo == true or (type(fnInfo) == 'table' and (getOption(fnInfo.option) or isPlayerInACLGroup(source, 'Admin'))) ) then
			local fn = _G

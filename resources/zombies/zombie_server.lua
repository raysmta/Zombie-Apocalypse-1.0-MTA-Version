...
ZombiePedSkins = {10,11,12,13,14,39 } --ALTERNATE SKIN LISTS FOR ZOMBIES (SHORTER LIST IS TEXTURED ZOMBIES ONLY)
...
local ZOMBIE_REWARD = 250 -- The amount of money rewarded to a player once killed
...
function outbreak(startedResource)
	...
	if startedResource == getThisResource() then
		call(getResourceFromName("scoreboard"), "scoreboardAddColumn", "Kills") --ADDS TO SCOREBOARD
...
function deanimated( ammo, attacker, weapon, bodypart )
	if (attacker) then
		if (getElementType ( attacker ) == "player") and (getElementType ( source ) == "ped") then
			if (getElementData (source, "zombie") == true) then
				local oldZcount = getElementData ( attacker, "Kills" )
				if oldZcount ~= false then
					setElementData ( attacker, "Kills", oldZcount+1  )
					triggerEvent ( "onZombieWasted", source, attacker, weapon, bodypart )
				else
					setElementData ( attacker, "Kills", 1  )
					triggerEvent ( "onZombieWasted", source, attacker, weapon, bodypart )				
				end
			end
		end
...
function givWep()
giveWeapon(zomb, 31, 10000 )
end
addEventHandler("onZombieSpawn", getResourceRootElement(), givWep)

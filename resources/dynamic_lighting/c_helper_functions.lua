...
---------------------------------------------------------------------------------------------------
function createLightShaderFiles(currLightNr, isLayer, isWrdSobel, isNightMod, isPedDiffuse, isShading, isDirLight)
	local isValid = true
	if currLightNr > 0 then
		isValid = createEffectFile('shaders/dynamic_wrd.fx', currLightNr, isLayer[1], isWrdSobel, isNightMod, false, isShading[1], isDirLight) and isValid
	else
		isValid = createEffectFile('shaders/dynamic_wrd.fx', currLightNr, isLayer[1], false, isNightMod, false, isShading[1], isDirLight) and isValid
	end
	isValid = createEffectFile('shaders/dynamic_veh.fx', currLightNr, isLayer[2], false, isNightMod, true, isShading[2], isDirLight) and isValid
	isValid = createEffectFile('shaders/dynamic_ped.fx', currLightNr, isLayer[3], false, isNightMod, isPedDiffuse, isShading[3], isDirLight) and isValid
	if isValid then
		...
		outputDebugString('Effect sobel WRD: '..tostring(isWrdSobel)..' Effect diffuse PED: '..tostring(isPedDiffuse)..' Dir Light Enable: '..tostring(isDirLight))
		outputDebugString('Effect nightMod: '..tostring(isNightMod))
		return true
	else
...
function createEffectFile(filePath, lightNr, isLayer, isSobel, isNightMod, isDiffuse, isShading, isDirLight)
	...
	if (theFile) then
		...
		if isShading then
			...
			if isDirLight then
				writeFileLine( theFile, ' PS.VertLight = saturate( createDirLight( normalize(PS.Normal), gDirLightDirection, gDirLightDiffuse)); ')
			end
			if isSobel then
		...
		if isDiffuse and isShading and not isLayer then
		...
		if isNightMod then
			writeFileLine( theFile, ' float diffGray = ( Diffuse.r + Diffuse.g + Diffuse.b )/3;\n Diffuse = lerp( float4(diffGray, diffGray, diffGray, Diffuse.a ), Diffuse, saturate( pow( gDayTime + 0.001, 1.4 ))); ')
		end
		writeFileLine( theFile, ' PS.Diffuse = Diffuse;\n return PS;\n } ')
		--ps
